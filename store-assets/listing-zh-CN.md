...
本扩展不使用远程代码。所有 JavaScript 均包含在扩展安装包内，不使用 `eval`、
动态代码执行或外部脚本。
...
- 个人身份信息：登录时用户主动输入并直接发送给小宇宙认证接口的手机号。
- 身份验证信息：小宇宙访问令牌、刷新令牌、设备标识和用户提供的 API Key。
- 网站内容：节目、单集、封面、音频地址、音频内容、转写稿和总结所需文本。

不勾选健康、财务、个人通讯、位置、网络记录和用户活动。本扩展不监控用户访问
的网页，不记录鼠标、滚屏或击键，也不做行为分析。
...
https://github.com/QWE38qwe/xiaoyuzhou-desktop/blob/main/PRIVACY.md
...
1. 安装后点击工具栏图标，扩展会打开独立桌面窗口。
2. 点击右上角“登录”，使用审核人员自己的小宇宙手机号获取一次性短信验证码。
   小宇宙只提供短信登录，因此没有可长期共享的固定测试密码。
3. 登录后在“发现”页播放任一单集，进入节目详情并查看单集列表。
4. 点击“下载音频”可使用 Chrome 默认下载目录验证基础下载能力。
5. “设置”页可检查 ASR Provider、AI Provider、Prompt 和保存目录配置。绝对
   路径、macOS Keychain、本地 Qwen3-ASR、云端 ASR 与 AI 总结依赖用户另行
   安装的开源 Native Host，并需要用户自己的 API Key。
6. Native Host 源码和安装说明：
   https://github.com/QWE38qwe/xiaoyuzhou-desktop

本扩展不包含开发者后端测试账号。若不登录，审核人员仍可打开独立窗口和设置页，
但小宇宙内容、订阅和播放需要通过小宇宙短信认证。
